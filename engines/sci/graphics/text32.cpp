...
//20140521
GfxText32E::GfxText32E(SegManager *segMan, GfxCache *fonts, GfxScreen *screen)
	: _segMan(segMan), _cache(fonts), _screen(screen) {
}

GfxText32E::~GfxText32E() {
}

reg_t GfxText32E::createScrollTextBitmap(Common::String text, reg_t textObject, uint16 maxWidth, uint16 maxHeight, reg_t prevHunk) {
	return createTextBitmapInternal(text, textObject, maxWidth, maxHeight, prevHunk);

}
reg_t GfxText32E::createTextBitmap(reg_t textObject, uint16 maxWidth, uint16 maxHeight, reg_t prevHunk) {
	reg_t stringObject = readSelector(_segMan, textObject, SELECTOR(text));
	// The object in the text selector of the item can be either a raw string
	// or a Str object. In the latter case, we need to access the object's data
	// selector to get the raw string.
...
reg_t GfxText32E::createTextBitmapInternal(Common::String &text, reg_t textObject, uint16 maxWidth, uint16 maxHeight, reg_t prevHunk) {
	// HACK: The character offsets of the up and down arrow buttons are off by one
	// in GK1, for some unknown reason. Fix them here.
	if (text.size() == 1 && (text[0] == 29 || text[0] == 30)) {
		text.setChar(text[0] + 1, 0);
	...
	GuiResourceId fontId = readSelectorValue(_segMan, textObject, SELECTOR(font));
	fontId = GetAppropriateFontId(fontId);
	GfxFont *font = _cache->getFont(fontId);
	bool dimmed = readSelectorValue(_segMan, textObject, SELECTOR(dimmed));
	int16 alignment = readSelectorValue(_segMan, textObject, SELECTOR(mode));
	uint16 foreColor = readSelectorValue(_segMan, textObject, SELECTOR(fore));
	...
	while (*txt) {
		...
		for (int i = 0; i < charCount; i++) {
			...
			curChar = (*(const byte *)txt++);
			if (font->isDoubleByte(curChar)) {
				curChar |= (*(const byte *)txt++) << 8;
				i++;
			}
...
void GfxText32E::disposeTextBitmap(reg_t hunkId) {
	_segMan->freeHunkEntry(hunkId);
}

void GfxText32E::drawTextBitmap(int16 x, int16 y, Common::Rect planeRect, reg_t textObject) {
	reg_t hunkId = readSelector(_segMan, textObject, SELECTOR(bitmap));
	drawTextBitmapInternal(x, y, planeRect, textObject, hunkId);
}

void GfxText32E::drawScrollTextBitmap(reg_t textObject, reg_t hunkId, uint16 x, uint16 y) {
	/*reg_t plane = readSelector(_segMan, textObject, SELECTOR(plane));
	Common::Rect planeRect;
	planeRect.top = readSelectorValue(_segMan, plane, SELECTOR(top));
	planeRect.left = readSelectorValue(_segMan, plane, SELECTOR(left));
...
void GfxText32E::drawTextBitmapInternal(int16 x, int16 y, Common::Rect planeRect, reg_t textObject, reg_t hunkId) {
	int16 backColor = (int16)readSelectorValue(_segMan, textObject, SELECTOR(back));
	// Sanity check: Check if the hunk is set. If not, either the game scripts
	// didn't set it, or an old saved game has been loaded, where it wasn't set.
	if (hunkId.isNull())
...
int16 GfxText32E::GetLongest(const char *text, int16 maxWidth, GfxFont *font) {
	uint16 curChar = 0;
	int16 maxChars = 0, curCharCount = 0;
	uint16 width = 0;
	...
	if (maxChars == 0) {
		...
		if (maxWidth == (width - font->getCharWidth(curChar))) {
			maxChars--;
			if (curChar > 0xFF)
				maxChars--;
			nextChar = curChar;
...
void GfxText32E::kernelTextSize(const char *text, int16 font, int16 maxWidth, int16 *textWidth, int16 *textHeight) {
	Common::Rect rect(0, 0, 0, 0);
	font = GetAppropriateFontId(font);
	Size(rect, text, font, maxWidth);
	*textWidth = rect.width();
	*textHeight = rect.height();
}

void GfxText32E::StringWidth(const char *str, GuiResourceId fontId, int16 &textWidth, int16 &textHeight) {
	fontId = GetAppropriateFontId(fontId);
	Width(str, 0, (int16)strlen(str), fontId, textWidth, textHeight, true);
}

void GfxText32E::Width(const char *text, int16 from, int16 len, GuiResourceId fontId, int16 &textWidth, int16 &textHeight, bool restoreFont) {
	uint16 curChar;
	textWidth = 0; textHeight = 0;
...
int GfxText32E::GetAppropriateFontId(int FontID)
{
	return 900;
}

int16 GfxText32E::Size(Common::Rect &rect, const char *text, GuiResourceId fontId, int16 maxWidth) {
	int16 charCount;
	int16 maxTextWidth = 0, textWidth;
	int16 totalHeight = 0, textHeight;
